...
const Auth = () => {
  ...
  const { user, accessToken, setUser, setAccessToken } =
  ...
  useEffect(() => {
    const validations = (hasDigitalCard) => {
      if (hasDigitalCard && location.pathname.startsWith("/create-card")) {
        ...
        openNotification(
          "Access Denied",
          "You have digital card already !",
          "info",
        );
      } else if (
        !hasDigitalCard &&
        location.pathname.startsWith("/manage-card")
      ) {
        navigate("/profile");
        openNotification(
          "Access Denied",
          "You don’t have a digital card",
          "error",
        );
      } else {
        const notAuthorizedPages = ["login", "signup"];
        const url = location.pathname.slice(1);
        const isExist = notAuthorizedPages.findIndex(
          (path) => url === path || url.length === 0,
        );
        if (isExist !== -1) {
          navigate("/profile");
    ...
    const checkAuthority = async () => {
      const validPages = ["login", "signup", "card"];
      if (isAuthorized === "pending") checkToken();
      else {
        if (!user || !accessToken) {
          const url = location.pathname.slice(1);
          const isValid = validPages.findIndex(
            (path) => url === path || url.length === 0,
          );
          if (isValid === -1) {
            navigate("/login");
            openNotification(
              "Access Denied",
              "Unauthorize, Please login to continue",
              "error",
            );
